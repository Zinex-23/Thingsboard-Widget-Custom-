...
(function () {
  ...
  function getMobileTableMetrics() {
    ...
    return {
      dateWidth: portraitExpanded ? (allDay ? 60 : 72) : (expandedMobile ? (allDay ? 56 : 64) : 92),
      headerHeight: portraitExpanded ? (allDay ? 32 : 36) : (expandedMobile ? (allDay ? 34 : 36) : 44),
      cellWidth: portraitExpanded ? (allDay ? 34 : 42) : (expandedMobile ? (allDay ? 30 : 36) : 56),
      cellHeight: portraitExpanded ? (allDay ? 36 : 42) : (expandedMobile ? (allDay ? 34 : 38) : 50),
      hourFont: portraitExpanded ? 10 : (expandedMobile ? (allDay ? 9 : 10) : 11),
      dateMainFont: portraitExpanded ? (allDay ? 10 : 11) : (expandedMobile ? (allDay ? 10 : 11) : 12),
      dateSubFont: portraitExpanded ? 9 : (expandedMobile ? 10 : 11),
      valueFont: portraitExpanded ? (allDay ? 9 : 10) : (expandedMobile ? (allDay ? 9 : 10) : 11),
      compact: expandedMobile && !portraitExpanded,
  ...
  function renderMobileHeatmap(map) {
    ...
    board.classList.toggle('is-portrait-expanded', metrics.portraitExpanded);
    ...
    var html = '<table class="eh-mobile-table' + (metrics.compact ? ' is-compact' : '') + (metrics.portraitExpanded ? ' is-portrait-expanded' : '') + '"><thead><tr>';
    html += '<th class="eh-mobile-corner" style="min-width:' + metrics.dateWidth + 'px;width:' + metrics.dateWidth + 'px;height:' + metrics.headerHeight + 'px;">' +
      '<span class="eh-mobile-corner-date">' + (metrics.portraitExpanded ? 'Hour' : 'Date') + '</span><span class="eh-mobile-corner-hour">' + (metrics.portraitExpanded ? 'Date' : 'Hour') + '</span></th>';

    if (metrics.portraitExpanded) {
      for (var dayHead = 0; dayHead < rows; dayHead++) {
        var headInfo = getCellDetails(startHour, dayHead, map[cellKey(startHour, dayHead)]);
        html += '<th class="eh-mobile-date-col" style="min-width:' + metrics.cellWidth + 'px;width:' + metrics.cellWidth + 'px;height:' + metrics.headerHeight + 'px;">' +
          '<span class="eh-mobile-date-main" style="font-size:' + metrics.dateMainFont + 'px;">' + headInfo.dateMain +
          '</span><span class="eh-mobile-date-sub" style="font-size:' + metrics.dateSubFont + 'px;">' + headInfo.dateSub + '</span></th>';
      }
      html += '</tr></thead><tbody>';

      for (var hour2 = startHour; hour2 <= endHour; hour2++) {
        var h0 = String(hour2).padStart(2, '0') + ':00';
        var h1 = String((hour2 + 1) % 24).padStart(2, '0') + ':00';
        html += '<tr>';
        html += '<th class="eh-mobile-date eh-mobile-row-head" style="min-width:' + metrics.dateWidth + 'px;width:' + metrics.dateWidth + 'px;">' +
          '<span class="eh-mobile-date-main" style="font-size:' + metrics.dateMainFont + 'px;">' + h0 +
          '</span><span class="eh-mobile-date-sub" style="font-size:' + metrics.dateSubFont + 'px;">' + h1 + '</span></th>';
        ...
        for (var dayIdx = 0; dayIdx < rows; dayIdx++) {
          ...
          html += '<td style="width:' + metrics.cellWidth + 'px;height:' + metrics.cellHeight + 'px;background:' + lc.css + ';">' +
            '<button class="' + cls + '" data-hour="' + hour2 + '" data-day="' + dayIdx +
      ...
      for (var hour = startHour; hour <= endHour; hour++) {
        html += '<th class="eh-mobile-hour" style="min-width:' + metrics.cellWidth + 'px;width:' + metrics.cellWidth + 'px;height:' + metrics.headerHeight + 'px;font-size:' + metrics.hourFont + 'px;">' + String(hour).padStart(2, '0') + '</th>';
      }
      html += '</tr></thead><tbody>';

      for (var dayIdx2 = 0; dayIdx2 < rows; dayIdx2++) {
        var sampleCell = map[cellKey(startHour, dayIdx2)];
        var dateInfo = getCellDetails(startHour, dayIdx2, sampleCell);
        html += '<tr>';
        ...
        for (var hour3 = startHour; hour3 <= endHour; hour3++) {
          var key2 = cellKey(hour3, dayIdx2);
          var cell2 = map[key2];
          var value2 = (cell2 && cell2.value) || 0;
          var lc2 = levelColor(value2, !!(cell2 && cell2.has), domainMax, PALETTE);
          var fg2 = (cell2 && cell2.has) ? ((lc2.level >= 0 && lc2.lum < 0.5) ? '#ffffff' : '#0f172a') : '#94a3b8';
          var cls2 = 'eh-mobile-cell-btn' + ((cell2 && cell2.has) ? '' : ' is-empty') + (mobileSelectedCellKey === key2 ? ' is-selected' : '') + (metrics.compact ? ' is-compact' : '');
          var label2 = (cell2 && cell2.has) ? formatInt(value2) : '—';
          html += '<td style="width:' + metrics.cellWidth + 'px;height:' + metrics.cellHeight + 'px;background:' + lc2.css + ';">' +
            '<button class="' + cls2 + '" data-hour="' + hour3 + '" data-day="' + dayIdx2 +
            '" style="width:100%;height:100%;background:' + lc2.css + ';color:' + fg2 + ';font-size:' + metrics.valueFont + 'px;">' + label2 + '</button></td>';
        }
        html += '</tr>';
      }
    }
