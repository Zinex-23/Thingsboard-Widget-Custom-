(function () {
    ...
    function extractEntityId(entityLike) {
        if (!entityLike) return null;
        if (typeof entityLike === 'string' || typeof entityLike === 'number') return String(entityLike);
        if (typeof entityLike.id === 'string' || typeof entityLike.id === 'number') return String(entityLike.id);
        if (entityLike.id && (typeof entityLike.id.id === 'string' || typeof entityLike.id.id === 'number')) {
            return String(entityLike.id.id);
        }
        return null;
    }

    function getCtxDatasourceEntityIds() {
        const datasources = self.ctx?.datasources || self.ctx?.dataSources || [];
        const ids = [];

        (Array.isArray(datasources) ? datasources : []).forEach(function (ds) {
            const id =
                extractEntityId(ds?.entity) ||
                extractEntityId(ds?.entityId) ||
                extractEntityId(ds?.datasource?.entity) ||
                extractEntityId(ds?.datasource?.entityId);
            if (id) ids.push(id);
        });
    ...
    function getStateSafe() {
        ...
        const selectedEntityId =
            extractEntityId(stateParams?.entityId) ||
            extractEntityId(stateParams?.selectedDeviceId) ||
            extractEntityId(stateParams?.id);
        const storeName = stateParams?.selectedDeviceName || stateParams?.name || 'All Devices';
        const datasourceEntityIds = getCtxDatasourceEntityIds();
        return { stateParams, deviceMode, entitiesFromState, selectedEntityId, datasourceEntityIds, storeName };
    }
    ...
    function makeSignature() {
        ...
        const { deviceMode, entitiesFromState, selectedEntityId, datasourceEntityIds } = getStateSafe();
        const dataKeys = getDataKeysSafe(sub);

        const allEntityIds = (entitiesFromState || []).map(function (e) {
            return extractEntityId(e);
        }).filter(Boolean).sort();
        const sigObj = {
            deviceMode: deviceMode || '',
            entityIds: allEntityIds,
            selectedEntityId: selectedEntityId || '',
            datasourceEntityIds: datasourceEntityIds || [],
    ...
    function processDataUpdate(seqToken) {
        ...
        const { stateParams, deviceMode, entitiesFromState, selectedEntityId, datasourceEntityIds, storeName } = getStateSafe();
        const isAllDevicesMode = deviceMode === 'ALL';
        const singleDatasourceSynced = !selectedEntityId || (datasourceEntityIds || []).includes(String(selectedEntityId));
        ...
        if (isAllDevicesMode && entitiesFromState.length > 1 && dataKeys.length > 0) {
        ...
        } else if (!isAllDevicesMode && selectedEntityId && !singleDatasourceSynced && dataKeys.length > 0) {
            if (activeFetchController) {
                try { activeFetchController.abort(); } catch (e) { }
            }
            const fetchToken = seqToken;
            activeFetchController = new AbortController();
            ...
            fetchDeviceTelemetry(
                ...
                .then(deviceData => {
                    ...
                    const datasets = buildFallbackSingleDeviceDatasets(
                        deviceData,
                        startTime,
                        endTime,
                        { isMonthMode, isYearMode, isHourlyGapMode, colorMap, labelMap }
    ...
    // =========================
    async function fetchDeviceTelemetry(deviceId, keys, startTime, endTime, signal) {
        if (!deviceId || !Array.isArray(keys) || !keys.length) return null;
        ...
        if (!response.ok) {
            throw new Error('Failed to fetch device telemetry: HTTP ' + response.status);
        }
    ...
    // =========================
    function buildFallbackSingleDeviceDatasets(deviceData, startTime, endTime, opts) {
        const results = [];
        ...
        let colorIndex = 0;
        Object.keys(deviceData || {}).forEach(function (keyName) {
            let points = (deviceData[keyName] || [])
                .filter(function (point) {
                    return (!startTime || point.ts >= startTime) && (!endTime || point.ts <= endTime);
                })
                .map(function (point) {
                    return {
                        t: Number(point.ts),
                ...
                .filter(function (point) {
                    return Number.isFinite(point.t) && point.y != null && !Number.isNaN(point.y);
                })
