...
var LOADING_TEXT = '読み込み中…';          // overlay text
...
(function () {
  ...
  function msToHuman(ms) {
    ...
    if (ms >= day)  return Math.round(ms/day)  + '日';
    if (ms >= hour) return Math.round(ms/hour) + '時間';
  ...
  function ensureLoadingOverlay(){
    ...
    if (!el) {
      ...
      el.innerHTML = `<div class="spin"></div><div class="txt">${LOADING_TEXT||'読み込み中…'}</div>`;
      root.appendChild(el);
    }
    return el;
  }
  ...
  self.typeParameters = function () {
    return {
      ...
      defaultDataKeysFunction: function () {
        return [{ name: 'ages', label: '年齢', type: 'timeseries' }];
      }
    };
  };
  ...
  function getTimeWindowText(ctx) {
    try {
      ...
      if (!stw) return '';
      if (stw.realtime && isFiniteNumber(stw.realtime.timewindowMs)) return '過去 ' + msToHuman(Number(stw.realtime.timewindowMs));
      ...
      if (stw.history && isFiniteNumber(stw.history.timewindowMs)) return '過去 ' + msToHuman(Number(stw.history.timewindowMs));
  ...
  function isMobileLayoutSize(cssW, cssH) {
    return cssW <= 640 || (cssW <= 820 && cssH > cssW * 1.05);
  }

  function getResponsiveMetrics(cssW, cssH, visibleBins) {
    var mobile = isMobileLayoutSize(cssW, cssH);
    var compact = mobile || cssW <= 560;
    return {
      mobile: mobile,
      compact: compact,
      margin: mobile
        ? { top: 8, right: 8, bottom: 40, left: 40 }
        : { top: 10, right: 12, bottom: 48, left: 56 },
      yTicks: mobile ? 3 : 4,
      yLabelFont: mobile ? 10 : 12,
      xLabelFont: mobile ? 9 : 11,
      axisTitleFont: mobile ? 10 : 12,
      axisTitleOffset: mobile ? 20 : 26,
      yAxisOffset: mobile ? 24 : 36,
      tickLength: mobile ? 3 : 4,
      tickStep: mobile ? Math.max(1, chooseNiceTickStep(visibleBins) * 2) : chooseNiceTickStep(visibleBins),
      tooltipOffset: mobile ? 8 : 4,
      radiusCap: mobile ? 4 : 6
    };
  }

  function getMobileDetailEl() {
    return document.getElementById('tb-hist-mobile-detail');
  }

  function setMobileDetailEmpty() {
    var el = getMobileDetailEl();
    if (!el) return;
    el.innerHTML =
      '<div class="tbw-mobile-detail-head">選択中の年齢</div>' +
      '<div class="tbw-mobile-detail-title">バーをタップ</div>' +
      '<div class="tbw-mobile-detail-meta">ヒストグラムの列をタッチして値を確認できます。</div>';
  }

  function renderMobileDetail(ai) {
    var el = getMobileDetailEl();
    if (!el) return;
    ...
    var index = (typeof ai === 'number' && ai >= 0) ? ai : _selectedAgeIndex;
    if (!(index >= 0 && index < state.labels.length)) {
      setMobileDetailEmpty();
      return;
    }
    ...
    for (var i = 0; i < state.seriesLabels.length; i++) {
      if (state.hiddenSeries && state.hiddenSeries[i]) continue;
      total += (state.counts2D[i] && state.counts2D[i][index]) ? state.counts2D[i][index] : 0;
    }
    for (var j = 0; j < state.seriesLabels.length; j++) {
      if (state.hiddenSeries && state.hiddenSeries[j]) continue;
      var value = (state.counts2D[j] && state.counts2D[j][index]) ? state.counts2D[j][index] : 0;
      rows.push(
        '<div class="tbw-mobile-detail-row">' +
          '<div class="tbw-mobile-detail-label">' +
            '<span class="tbw-mobile-detail-swatch" style="background:' + state.colors[j % state.colors.length] + '"></span>' +
            '<span>' + state.seriesLabels[j] + '</span>' +
          '</div>' +
          '<div class="tbw-mobile-detail-value">' + value + (total ? ' (' + formatPct(value, total) + ')' : '') + '</div>' +
        '</div>'
      );
    }
    ...
    el.innerHTML =
      '<div class="tbw-mobile-detail-head">選択中の年齢</div>' +
      '<div class="tbw-mobile-detail-title">年齢 ' + state.labels[index] + '</div>' +
      '<div class="tbw-mobile-detail-meta">合計: ' + total + '</div>' +
      (rows.length ? '<div class="tbw-mobile-detail-list">' + rows.join('') + '</div>' : '<div class="tbw-mobile-detail-empty">データなし</div>');
  }

  function getDataAtPosition(mx, my) {
    var L = _layout;
    if (!L) return null;
    ...
    var ai = L.zStart + col;
    if (ai < L.zStart || ai > L.zEnd) return null;
    ...
    return {
      ai: ai,
      col: col,
      totalAtAge: totalAtAge,
      hoveredSi: hoveredSi,
      baseline: baseline
    };
  }

  function buildTooltipHtml(ai, totalAtAge) {
    var Slen = state.seriesLabels.length;
    var ageVal = state.labels[ai];
    var lines = [];
    lines.push('<div style="font-weight:600;margin-bottom:4px">年齢: ' + ageVal + '</div>');
    ...
    if (Slen > 1) lines.push('<div style="margin-top:4px;opacity:.9">合計: <b>' + totalAtAge + '</b></div>');
    return lines.join('');
  }
  ...
  function writeLegend() {
    ...
    for (var i=0;i<state.seriesLabels.length;i++){
      ...
      txt.appendChild(document.createTextNode(state.seriesLabels[i]+' — 合計: '+(state.totals[i]||0)));
    ...
    if (!state.seriesLabels.length){
      ...
      empty.appendChild(document.createTextNode('データなし'));
  ...
  function drawHistogramStacked() {
    ...
    var metrics = getResponsiveMetrics(cssW, cssH, visibleBins);
    ...
    ctx.font = metrics.yLabelFont + 'px system-ui, -apple-system, Segoe UI, Arial, sans-serif';
    ctx.textAlign = 'right';
    ...
    for (var ai=zStart; ai<=zEnd; ai++){
      ...
      ctx.strokeStyle = '#cbd5e1';
      ctx.beginPath(); ctx.moveTo(x, margin.top + plotH); ctx.lineTo(x, margin.top + plotH + metrics.tickLength); ctx.stroke();
      ctx.fillStyle = '#64748b';
      ctx.font = metrics.xLabelFont + 'px system-ui, -apple-system, Segoe UI, Arial, sans-serif';
      ctx.fillText(String(ageVal), x, margin.top + plotH + 6);
    }

    // Axis titles
    ctx.save();
    ctx.font = metrics.axisTitleFont + 'px system-ui, -apple-system, Segoe UI, Arial, sans-serif';
    ctx.fillStyle = '#475569';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
    var xTitle = state.zoom.enabled ? ('年齢（' + labels[zStart] + '～' + labels[zEnd] + '）') : '年齢';
    ctx.fillText(xTitle, margin.left + plotW / 2, margin.top + plotH + metrics.axisTitleOffset);
    ctx.restore();

    ctx.save();
    ctx.translate(margin.left - metrics.yAxisOffset, margin.top + plotH / 2);
    ...
    _layout = {
      ...
      tooltipOffset: metrics.tooltipOffset
    };
    ...
    for (var ai2 = zStart; ai2 <= zEnd; ai2++) {
      ...
      for (var si = 0; si < Slen; si++) {
        ...
        if (si === lastNonZero[ai2]) {
          drawRoundedTopRect(ctx, xBar, yBar, Math.max(1, barW), hAnim, Math.min(metrics.radiusCap, barW * 0.35));
        } else {
  ...
  async function runUpdate(mySeq) {
    ...
    try {
      ...
      if (!GROUP_BY_DATASOURCE) {
        ...
        var series = [];
        if (SERIES === 'male' || SERIES === 'both')   series.push({ label: '男性',   ages: maleGlobal });
        if (SERIES === 'female' || SERIES === 'both') series.push({ label: '女性', ages: femaleGlobal });
      ...
      for (var gi2 = 0; gi2 < groups.length; gi2++) {
        ...
        if (SERIES === 'male' || SERIES === 'both')   seriesOld.push({ label: g2.label + ' / 男性',   ages: maleAll });
        if (SERIES === 'female' || SERIES === 'both') seriesOld.push({ label: g2.label + ' / 女性', ages: femaleAll });
      }
  ...
  function attachHoverEvents() {
    ...
    function updateInteraction(clientX, clientY, persistSelection) {
      if (!_layout) return;

      var rect = canvas.getBoundingClientRect();
      var mx = clientX - rect.left;
      var my = clientY - rect.top;
      var info = getDataAtPosition(mx, my);
      if (!info) {
      ...
      var L = _layout;
      var cx = L.margin.left + (info.col + 0.5) * L.barW;
      var cy = L.margin.top + L.plotH - info.baseline - L.tooltipOffset;
      if (persistSelection || L.mobile) {
        _selectedAgeIndex = info.ai;
        renderMobileDetail(info.ai);
      }

      if (!L.mobile) {
        showTooltip(buildTooltipHtml(info.ai, info.totalAtAge), cx, cy);
      } else {
        hideTooltip();
      }
      _hover.ai = info.ai;
      _hover.si = info.hoveredSi;
    }

    canvas.addEventListener('mousemove', function (ev) {
      updateInteraction(ev.clientX, ev.clientY, false);
    });

    canvas.addEventListener('click', function (ev) {
      updateInteraction(ev.clientX, ev.clientY, true);
    });
