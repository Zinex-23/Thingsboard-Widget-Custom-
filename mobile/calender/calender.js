...
(function () {
  ...
  function isMobileLayout() {
    ...
    var width = root.clientWidth || root.offsetWidth || window.innerWidth || 0;
    var height = root.clientHeight || root.offsetHeight || window.innerHeight || 0;
    return width <= 760 || (width <= 920 && height > width * 1.05);
  }

  function setMobileDetailEmpty() {
    var detail = rootEl().querySelector('#tb-cal-mobile-detail');
    if (!detail) return;
    if (!isMobileLayout()) {
      detail.innerHTML = '';
      return;
    ...
    detail.innerHTML =
      '<div class="tb-cal__mobile-detail-head">Selected Day</div>' +
      '<div class="tb-cal__mobile-detail-title">Tap a day</div>' +
      '<div class="tb-cal__mobile-detail-meta"><div>Touch a calendar cell to inspect its value and comparison.</div></div>';
  }

  function renderMobileDetail(data) {
    var detail = rootEl().querySelector('#tb-cal-mobile-detail');
    if (!detail) return;
    if (!isMobileLayout()) {
      detail.innerHTML = '';
    ...
    var lines = [];
    lines.push('<div>Current: <b>' + data.current + '</b></div>');
    if (data.compareLabel && data.compareValue != null) {
      lines.push('<div>' + data.compareLabel + ': <b>' + data.compareValue + '</b></div>');
    }
    if (data.changeLabel) {
      lines.push('<div>Change: <span style="color:' + data.changeColor + '"><b>' + data.changeLabel + '</b></span></div>');
    }

    detail.innerHTML =
      '<div class="tb-cal__mobile-detail-head">Selected Day</div>' +
      '<div class="tb-cal__mobile-detail-title">' + data.title + '</div>' +
      '<div class="tb-cal__mobile-detail-meta">' + lines.join('') + '</div>';
  }
  ...
  function applyResponsiveLayout() {
    ...
    var keyWrap = root.querySelector('.tb-key-wrap');
    var compareWrap = root.querySelector('.tb-compare-wrap');
    if (!header || !toolbar) return;

    if (isMobileLayout()) {
      header.style.display = 'grid';
      header.style.gridTemplateColumns = '1fr';
      header.style.columnGap = '0';
      header.style.rowGap = '10px';

      toolbar.style.display = 'grid';
      toolbar.style.gridTemplateColumns = '1fr';
      toolbar.style.gap = '10px';
      toolbar.style.width = '100%';
      toolbar.style.gridColumn = '1';

      if (keyWrap) {
        keyWrap.style.gridColumn = 'auto';
        keyWrap.style.justifySelf = 'stretch';
        keyWrap.style.width = '100%';
      }
      if (compareWrap) {
        compareWrap.style.gridColumn = 'auto';
        compareWrap.style.justifySelf = 'stretch';
        compareWrap.style.width = '100%';
        compareWrap.style.marginLeft = '0';
      }
    } else {
      header.style.display = 'grid';
      header.style.gridTemplateColumns = '1fr minmax(280px, 520px) 1fr';
      header.style.columnGap = '15px';
      header.style.rowGap = '0';

      toolbar.style.display = 'contents';
      toolbar.style.gridTemplateColumns = '';
      toolbar.style.gap = '';
      toolbar.style.width = '';
      toolbar.style.gridColumn = '';

      if (keyWrap) {
        keyWrap.style.gridColumn = '2';
        keyWrap.style.justifySelf = 'center';
        keyWrap.style.width = '';
      }
      if (compareWrap) {
        compareWrap.style.gridColumn = '3';
        compareWrap.style.justifySelf = 'center';
        compareWrap.style.width = '';
        compareWrap.style.marginLeft = '0';
      }
    }
  }
  ...
  function compareFetchRangeMs(y, m, mode) {
    var current = monthRangeMs(y, m);
    var firstDay = new Date(y, m, 1);
    var lastDay = new Date(y, m + 1, 0);
    var prevFirst = getPrevAnchorDate(firstDay, mode || S.compareMode);
    var prevLast = getPrevAnchorDate(lastDay, mode || S.compareMode);

    var start = startOfDay(prevFirst).getTime();
    var end = current.end;
    var prevLastEnd = endOfDay(prevLast).getTime();
    if (prevLastEnd > end) end = prevLastEnd;
  ...
  function renderGrid() {
    ...
    for (var i = 0; i < 42; i++) {
      ...
      if (inMonth) {
        ...
        var dateStrBase = d.toLocaleDateString(S.locale, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' });
        var mobileDetailData = {
          title: dateStrBase,
          current: hasVal ? fmtNumber(val) : 'No data',
          compareLabel: hasPrev ? anchorLabel : '',
          compareValue: hasPrev ? fmtNumber(prevVal) : null,
          changeLabel: showDelta ? ((pctDay > 0 ? '+' : '') + fmtPercent(pctDay)) : '',
          changeColor: pctDay > 0 ? S.deltaUpColor : (pctDay < 0 ? S.deltaDownColor : S.deltaZeroColor)
        };
        ...
        cell.addEventListener('click', function (detailData, ms) {
          return function () {
            selectedDayMs = ms;
            renderMobileDetail(detailData);
            renderGrid();
          };
        }(mobileDetailData, dMs));
      }
  ...
  async function refreshNow(needRefetchAll, token) {
    ...
    try {
      ...
      if (mode === 'ALL') {
        ...
        var range = compareFetchRangeMs(viewYear, viewMonth, S.compareMode);
      ...
      if (SINGLE_FETCH_USES_API) {
        ...
        if (singleId) {
          ...
          __activeFetchController = new AbortController();
          var rangeS = compareFetchRangeMs(viewYear, viewMonth, S.compareMode);
  ...
  function setupResizeObserver() {
    var root = rootEl();
    if (!root || typeof ResizeObserver === 'undefined' || resizeObserver) return;
