...
function ensureSharedDropdownManager() {
  ...
  var mgr = {
    ...
    position: function (anchorEl) {
      ...
      var anchorWidth = Math.max(160, Math.round(a.width * scaleX));
      var maxWidth = Math.max(160, vw - gap * 2);
      var targetWidth = Math.min(anchorWidth, maxWidth);

      dd.style.width = targetWidth + 'px';
      dd.style.maxWidth = targetWidth + 'px';

      var ddW = dd.offsetWidth || targetWidth;
      var ddH = dd.offsetHeight || 200;

      if (left + ddW + gap > vw) left = Math.max(gap, vw - ddW - gap);
      if (top + ddH + gap > vh) top = Math.max(gap, f.top + (a.top * scaleY) + vvTop - ddH - gap);
