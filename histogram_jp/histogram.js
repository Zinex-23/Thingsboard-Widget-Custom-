  ...
  async function fetchDeviceGroups(deviceId, startTs, endTs, urlKeys, token, parentSignal){
    ...
    try{
      ...
      if (DEBUG && timed.isTimedOut()) {
        try { console.warn('[histogram_jp] telemetry timeout for device', deviceId); } catch(_){}
      }
      return [];
    } finally {
      timed.cleanup();
  ...
  async function runUpdate(mySeq) {
    ...
    try {
      ...
      if (!GROUP_BY_DATASOURCE) {
        ...
        if (SERIES === 'both' && unknownGlobal.length) series.push({ label: '不明', ages: unknownGlobal });
      ...
      for (var gi2 = 0; gi2 < groups.length; gi2++) {
        ...
        if (SERIES === 'both' && unknownAll.length)   seriesOld.push({ label: g2.label + ' / 不明', ages: unknownAll });
      }
      ...
      if (DEBUG) {
        try { console.error('[histogram_jp] runUpdate error', _e); } catch(_e2){}
      }
    } finally {
      stopLoading(mySeq);
    }
